...
export const CompanyManager = ({
    ...
    const companyList = useMemo(() => {
        return Object.entries(companies).map(([id, c]) => {
            ...
            const leadCount = leads.filter(l => l.Company === c.Company).length;
            ...
            // Helper to parse "14,000" -> 14000 (Handle strings with commas/spaces)
            const getEmp = (v) => Number(String(v || '0').replace(/[^0-9]/g, ''));

            // Sort by Employees Descending, then by Lead Count
            return (getEmp(b.Employees) - getEmp(a.Employees)) || (b.leadCount - a.leadCount);
        });
    }, [companies, leads, searchQuery, filter]);
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 text-xs font-bold text-slate-500 uppercase border-b border-slate-200">
                            <tr>
                                <th className="px-6 py-3 w-1/5">Company Name</th>
                                ...
                                return (
                                    <tr key={c.id} className={`group hover:bg-slate-50 transition-colors ${isEditing ? 'bg-indigo-50/30' : ''}`}>
                                        ...
                                        <td className="px-6 py-3 text-center">
                                            <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-bold ${c.leadCount > 0 ? 'bg-slate-100 text-slate-600' : 'bg-red-50 text-red-500'}`}>
                                                {c.leadCount}
                                            </span>
                                        </td>
